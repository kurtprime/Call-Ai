...
const pricingCardVariants = cva("rounded-lg p-4 py-6 w-full", {
  variants: {
    variant: {
      ...
      highlighted: "bg-linear-to-br from-chart-5 to-primary text-accent",
    },
    defaultVariants: {
      variant: "default",
    },
...
const pricingCardIconsVariants = cva("size-5", {
  variants: {
    variant: {
      default: "fill-primary text-accent",
      highlighted: "fill-accent text-accent-foreground",
    },
    defaultVariants: {
      variant: "default",
    },
...
const pricingCardSecondaryTextVariants = cva("text-accent", {
  variants: {
    variant: {
...
const pricingCardBadgeVariants = cva(
  "text-accent-foreground text-xs font-normal p-1",
  {
    variants: {
      variant: {
...
export const PricingCard = ({
  ...
  return (
    <div className={cn(pricingCardVariants({ variant }), className, "border")}>
      <div className="flex items-end gap-x-4 justify-between">
        <div className="flex flex-col gap-y-2 ">
          <div className="flex items-center gap-x-2">
            ...
            {badge ? (
              <Badge className={cn(pricingCardBadgeVariants({}))}>
                {badge}
              </Badge>
            ) : null}
          </div>
      ...
      <div className="py-6">
        <Separator className="opacity-10 text-accent-foreground" />
      </div>
